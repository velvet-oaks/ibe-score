...
export class UsebioComponent {
	...
	uploadFiles(): void {
		this.message = [];
		console.log(
			'this.selectedFiles, after uploadFiles() clicked: ',
			this.selectedFiles
		);
		if (this.selectedFiles) {
			this.uploading = true;
			...
			let xmlFile;
			this.uploadDownload.uploadUSEBIO(this.filesArray).subscribe({
				next: (event: any) => {
					// console.log(
					// 	'uploadFiles() event: ',
					// 	event + '\n',
					...
					if (event instanceof HttpResponse) {
						...
						if (victorsResponse === 'SUCCESS') {
							...
							console.log("uploadFiles() victor's Response: ", victorsResponse);
